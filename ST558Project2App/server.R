...
#Get an exact match by making sure the name equals exactly the inputted country name. Without this, if you chose "Dominica" for example it would return both Dominica and Dominican Repbulic. I only want it to return one country for this section.
  ...
  #Convert to tibble. Return only rows where there was an exact match found. Return all columns. (This will be specified later).
...
get_region_info <- function(region_name, chosen_cols){
  ...
  #Name the columns of the tibble. See get_country_info for more information about is.null being used.
  tibble_info <- tibble(
    Country_Name = tibble_infoname$common,
    Capital = ifelse(is.null(tibble_info$capital), NA, 
                      as.character(tibble_info$capital)),
    Region = tibble_info$region,
    Subregion = ifelse(is.null(tibble_info$subregion), NA, 
...
output$filtered_data <- renderDataTable({
  ...
  output$contingency_tab <- renderTable({
    if(input$summary == "Contingency Tables" & input$con_tab == "One Way" & !(input$con1_var %in% input$cols)){
