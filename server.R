...
get_country_info <- function(country_name, chosen_cols){
  ...
  #Based on documentation provided on API, paste together formatting of initial url and country name. Use the URLencode below to deal with countries like United States and such that have spaces so then it will use %20 and countries that may have special characters in their names (Aland Islands was one of them).
  encoded_name <- URLencode(country_name)
  url <- paste0(start_url,
                "name/",
                encoded_name)
  #Use the GET function from the httr package to contact API.
  return_data <- GET(url)
...
#Create a get subregion info function that takes in a subregion name and columns and returns all countries from that subregion.
...
get_pop_info <- function(min_value = 0, max_value = 20000000000, chosen_cols){
  ...
  tibble_info <- tibble_info |>
    #Only get those rows that that have a population greater than or equal to the minimum value and less than or equal to the maximum value.
...
get_area_info <- function(min_area = 100, max_area = 100000, chosen_cols){
  ...
  tibble_info <- tibble_info |>
    #Only get those rows that that have an area greater than or equal to the minimum value and less than or equal to the maximum value.
...
get_bar <- function(data_input, cat1, cat2){
  ...
  ggraph + geom_bar() +
    #Adjust the x axis labels because they were originally going on top of each other so adjust vertically so that you can see them clearly.
...
get_histogram <- function(input_data, x_val, y_val){
  ...
  g <- ggplot(input_data |> drop_na(y_val), aes_string(x = x_val, fill = y_val)) +
    ...
    #Facet wrap on the categorical variable to create all possible graphs of the quantitative variable at all levels of the categorical variable.
    facet_wrap(y_val) +
...
get_scatter <- function(input_data, facet_var){
  #Drop any missing variables for the facet_var. Use aes_string since input is being passed as a string. x and y are static as population and area since those are the only quantitative variables in this dataset. Color by the categorical variable. 
...
server <- function(input, output, session) {
  ...
  output$choice <- renderUI({
    ...
    #If the "Choose Filtering Method Box" (internally stored as filter) is selected as Subregion, then display another dropdown box for selecting the subregion name. Have the display name be "Subegion Name" and store internally as subregion.
  ...
  #Create a reactive context for filtered_data. This will allow us to apply different functions based on the filter selected (Country Name, Region, Language, Population, or Area) and then take the inputs of those (say the name of the country for example) and use those as the inputs of the functions that we originally created. We can then store this dataset to render a data table (stay tuned!) and also use this on the Data Exploration Tab to make graphs and such. All of them also have input$cols as the last input. This comes from the UI file where we selected the columns with a checkbox and then we are passing those selections through here.
filtered_data <- reactive({
  ...
  if (input$filter == "Country Name") {
    ...
    #If Subregion was selected, apply get_subregion_info function. Use the Subregion Name (internally stored as subregion) as the first input.
  } else if(input$filter == "Subregion") {
...
#There is an initial dropdown box called "Summary Type" that is internally stored as summary. If summary is on "Contingency Tables", get a second drop down box named "Type of Contingency Table" that is internally stored as con_tab. Give options for a one way or two way contingency table.    
 ...
 #If summary is set to "Graphical Displays", display another dropdown box with options for the choices for the graphical displays. Call this dropdown box "Type of Graph" and internally store as graph.
  ...
  output$con1_opt <- renderUI({
    ...
    #If user selects both of the following, create a dropdown box for the variable to create the table for. Internally store this variable as con1_var to use when actually making the table next.
  ...
  contingency_table <- reactive({
    ...
    if(input$summary == "Contingency Tables" & input$con_tab == "One Way"){
      ...
      #Give meaningful column names because just true/false is not helpful.
...
#If Tree Map is selected from the graphical display, then create a dropdown box to select the quantitative variable and the categorical variable to facet with.
  output$treemap_opt <- renderUI({
    ...
    if (input$summary == "Graphical Displays" && input$graph == "Tree Map") {
      ...
      tagList(
        #Give options for quantitative variable and categorical variable.
  ...
  treemap_graph <- reactive({
    ...
    if(input$summary == "Graphical Displays" && input$graph == "Tree Map" & tree_var == "Population"){
      ggplot(filtered_data() |> drop_na(cat_var), aes(area = Population, fill = Population, label = Country_Name))+
        ...
        #Facet by the choice chosen for the categorical variable.
        facet_wrap(cat_var)
...
#Create the options for the histogram if the "Histograms" option was selected.   
  output$hist_opt <- renderUI({
    ...
    if (input$summary == "Graphical Displays" && input$graph == "Histograms"){
      tagList(
        #Have two radio Buttons of options for a numeric variable and a categorical variable. Internally store these as num_hist and cat_hist with display names of "Choose a numeric variable" and "Choose a categorical variable to facet".
